...
public interface CitaRepository extends JpaRepository<Cita, Long> {
    ...
    @Query("SELECT new com.vet_saas.modules.dashboard.dto.TopServicioDto(" +
            ...
            "COALESCE(SUM(CASE WHEN c.estado = com.vet_saas.modules.appointment.model.AppointmentStatus.COMPLETADA THEN c.servicio.precio ELSE 0 END), 0)) " +
            "FROM Cita c JOIN c.servicio s " +
            "WHERE c.empresa.id = :empresaId " +
            "AND c.estado NOT IN (com.vet_saas.modules.appointment.model.AppointmentStatus.RECHAZADA, com.vet_saas.modules.appointment.model.AppointmentStatus.CANCELADA) " +
    ...
    // una cita rechazada o futura.
    @Query("SELECT c.cliente.id, COUNT(c), " +
            "COALESCE(SUM(CASE WHEN c.estado = com.vet_saas.modules.appointment.model.AppointmentStatus.COMPLETADA THEN c.servicio.precio ELSE 0 END), 0), " +
            "MAX(CASE WHEN c.estado = com.vet_saas.modules.appointment.model.AppointmentStatus.COMPLETADA THEN c.fechaProgramada ELSE NULL END) " +
            "FROM Cita c " +
            "WHERE c.empresa.id = :empresaId " +
    ...
    @Query("SELECT DISTINCT c.cliente.id FROM Cita c " +
            ...
            "AND c.estado IN (com.vet_saas.modules.appointment.model.AppointmentStatus.CONFIRMADA, com.vet_saas.modules.appointment.model.AppointmentStatus.COMPLETADA)")
